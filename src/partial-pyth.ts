...
async function runPartialLiquidator() {
  const slackWebhookUrl =
    process.env.SLACK_WEBHOOK_URL || 'https://webhook.slack.com';
  ...
  for (const reserve of parsedReserveMap.values()) {
    wallets.set(
      ...
      await findLargestTokenAccountForOwner(
        connection,
        payer,
        reserve.reserve.liquidity.mintPubkey,
      ),
    );
    wallets.set(
      ...
      await findLargestTokenAccountForOwner(
        connection,
        payer,
        reserve.reserve.collateral.mintPubkey,
      ),
    );
  }
  ...
  while (true) {
    try {
      ...
      if (unhealthyObligations.length) {
        console.log(
      ...
      for (const unhealthyObligation of unhealthyObligations) {
        notify(
          `Liquidating obligation account ${unhealthyObligation.obligation.publicKey.toBase58()} which is owned by ${unhealthyObligation.obligation.owner.toBase58()} with risk factor: ${
            unhealthyObligation.riskFactor
          }
           which has borrowed ${
             unhealthyObligation.loanValue
           } with liquidation borrowed value at ${
            unhealthyObligation.obligation.unhealthyBorrowValue
          } ...`,
        );
        await liquidateAccount(
          ...
          programId,
          payer,
          unhealthyObligation.obligation,
          parsedReserveMap,
          wallets,
        );
      }
...
function redeemRemainingCollaterals(
  parsedReserveMap: Map<string, EnrichedReserve>,
  programId: PublicKey,
  connection: Connection,
  payer: Account,
  wallets: Map<string, { publicKey: PublicKey; tokenAccount: Wallet }>,
) {
  parsedReserveMap.forEach(async (reserve) => {
    const lendingMarket: PublicKey = parsedReserveMap.values().next().value
...
async function readSymbolPrice(
  ...
  const pythData = await connection.getAccountInfo(
...
async function readTokenPrices(
  connection,
  allReserve: Map<string, EnrichedReserve>,
...
async function getUnhealthyObligations(
  connection: Connection,
  programId: PublicKey,
  allReserve: Map<string, EnrichedReserve>,
) {
  ...
  const sortedObligations = obligations
    ...
    .map((obligation) =>
      generateEnrichedObligation(obligation, tokenToCurrentPrice, allReserve),
    )
  ...
  //     console.log(
  //       `Risk factor: ${ob.riskFactor.toFixed(
  //         4,
  //       )} borrowed amount: ${scaleToNormalNumber(
  //         ob.loanValue,
  //         10,
  //       )} deposit amount: ${scaleToNormalNumber(ob.collateralValue, 10)}
  ...
  //   console.log(
  //     `name: ${reserveLookUpTable[token]} price: ${scaleToNormalNumber(
  //       price,
  //       10,
  //     )}`,
  //   );
...
function generateEnrichedObligation(
  ...
  tokenToCurrentPrice: Map<string, BN>,
  allReserve: Map<string, EnrichedReserve>,
): EnrichedObligation {
  let loanValue = ZERO;
  ...
  for (const borrow of obligation.borrows) {
    ...
    let totalPriceWad = borrow.borrowedAmountWads
      .mul(tokenPriceWad)
      .div(WAD)
      .div(TEN.pow(new BN(reserve.liquidity.mintDecimals)));
  ...
  for (const deposit of obligation.deposits) {
    ...
    let name = reserveLookUpTable[reservePubKey];
    let reserve = allReserve.get(reservePubKey)!.reserve;
    let totalSupply = reserve.liquidity.availableAmount.add(
      wadToBN(reserve.liquidity.borrowedAmountWads),
    );
    let collateralTotalSupply = reserve.collateral.mintTotalSupply;
    // In percentage
    let liquidationThreshold = reserve.config.liquidationThreshold!;
    let tokenPriceWad = tokenToCurrentPrice.get(reservePubKey)!;
    let totalPriceWad = deposit.depositedAmount
      .mul(totalSupply)
      .mul(tokenPriceWad)
      .mul(new BN(liquidationThreshold))
      .div(collateralTotalSupply)
      .div(new BN(100))
      .div(TEN.pow(new BN(reserve.liquidity.mintDecimals)));
  ...
  const riskFactor =
    collateralValue === ZERO || loanValue === ZERO
      ? 0
      : scaleToNormalNumber(loanValue.mul(WAD).div(collateralValue), 18);
...
async function liquidateAccount(
  ...
  toRefreshReserves.forEach((reserve) => {
    transaction.add(refreshReserveInstruction(parsedReserveMap.get(reserve)!));
  });

  // TODO: choose a more sensible value
  const repayReserve: EnrichedReserve | undefined = parsedReserveMap.get(
    obligation.borrows[0].borrowReserve.toBase58(),
  );
  const withdrawReserve: EnrichedReserve | undefined = parsedReserveMap.get(
    obligation.deposits[0].depositReserve.toBase58(),
  );
  ...
  if (
    repayReserve.reserve.liquidity.mintPubkey.toBase58() !== SOL_MINT &&
  ...
  const repayWallet = await findLargestTokenAccountForOwner(
    connection,
    payer,
    repayReserve.reserve.liquidity.mintPubkey,
  );
  ...
  const transferAuthority =
    repayReserve.reserve.liquidity.mintPubkey.toBase58() !== SOL_MINT
      ...
      : await liquidateByPayingSOL(
          ...
          wallets.get(withdrawReserve.reserve.collateral.mintPubkey.toBase58())!
            .publicKey,
          repayReserve,
          withdrawReserve,
          obligation,
...
async function liquidateByPayingToken(
  ...
  transaction.add(
    ...
    liquidateObligationInstruction(
      ...
      lendingMarketAuthority,
      transferAuthority.publicKey,
      withdrawReserve.reserve.deposit_staking_pool_option === 1
        ? withdrawReserve.reserve.deposit_staking_pool
        : undefined,
      withdrawReserve.reserve.deposit_staking_pool_option === 1
...
async function redeemCollateral(
  ...
  transaction.add(
    ...
    redeemReserveCollateralInstruction(
      ...
      wallets.get(withdrawReserve.reserve.liquidity.mintPubkey.toBase58())!
        .publicKey!,
      withdrawReserve.publicKey,
      withdrawReserve.reserve.collateral.mintPubkey,
